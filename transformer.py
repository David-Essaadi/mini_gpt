...
class Head(nn.Module):
    def __init__(self, head_size, context_length, *args, **kwargs) -> None:
        ...
        self.key = nn.Linear(n_embedding_dims, head_size, bias=False)
        self.query = nn.Linear(n_embedding_dims, head_size, bias=False)
        self.value = nn.Linear(n_embedding_dims, head_size, bias=False)
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads, head_size, context_length, *args, **kwargs) -> None:
        ...
        self.proj = nn.Linear(n_embedding_dims, n_embedding_dims)
        self.dropout = nn.Dropout(dropout_ratio)
...
class FeedForward(nn.Module):
    def __init__(self, n_embedding_dims, *args, **kwargs) -> None:
        ...
        self.net = nn.Sequential(
            nn.Linear(n_embedding_dims, n_embedding_dims * 4),
            nn.ReLU(),
            nn.Linear(n_embedding_dims * 4, n_embedding_dims),
...
class Block(nn.Module):
    def __init__(
        ...
        self.layer_norm_1 = nn.LayerNorm(n_embedding_dims)
        self.layer_norm_2 = nn.LayerNorm(n_embedding_dims)
